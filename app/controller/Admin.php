...
class Admin extends BaseController
{
    ...
    public function getsystem() //获取全部系统配置
    {
        if (Request::instance()->isPost()) //判断是否isPost请求
        {
            ...
            $this->data['web']['bdogepi_presale_end'] = date('Y-m-d H:i:s',AdminSystemModel::where('key', 'bdogepi_presale_end')->find()['value']);
            $this->data['web']['bdogepi_presale_percentage'] = AdminSystemModel::where('key', 'bdogepi_presale_percentage')->find()['value'];
            return json(['data' => $this->data, 'code' => $this->code, 'message' => $this->message]); //返回JSON参数 
        }
    }
